...
// ─── Environment Helper ───
// Returns TEST_URL or DEPLOYED_URL based on the USE_TEST_URL flag
String get activeBaseUrl {
  final useTest = dotenv.env['USE_TEST_URL']?.toLowerCase() == 'true';
  if (useTest) {
    return dotenv.env['TEST_URL'] ??
        'https://furtive-chrissy-reparably.ngrok-free.dev';
  }
  return dotenv.env['DEPLOYED_URL'] ??
      'https://supply.bharatintelligence.ai';
}

// ─── FCM API Endpoints (derived from activeBaseUrl) ───
String get fcmUserTokensUrl => '$activeBaseUrl/api/fcm/user-tokens/';
String get fcmSaveTokenUrl => '$activeBaseUrl/api/save-fcm-token/';
String get fcmUpdateTokenUrl => '$activeBaseUrl/api/fcm/update-token/';

// ─── Background Handler (top-level, cannot use dotenv) ───
...
// ─── Shared logic for both foreground and background ───
...
class FirebaseMsg {
  ...
  Future<void> initFCM(String userId, String mobileNumber) async {
    ...
    print("🌐 Active Base URL: $activeBaseUrl");
    ...
    if (token != null) {
      ...
      if (cachedToken != token) {
        ...
        await fetchServerTokenData(mobileNumber);

        if (serverData != null && serverData['success'] == true) {
          List devices = serverData['devices'] ?? [];
          bool isTokenRegistered =
          devices.any((device) => device['fcm_token'] == token);
          String? primaryToken = serverData['primary_token'];

          if (!isTokenRegistered || primaryToken != token) {
            print(
                "Existing number found. Token mismatch or not primary. Updating...");
            await updateTokenOnBackend(mobileNumber, token);
          } else {
            print(
                "Server already has this token as primary. No update needed.");
          }
        } else {
          print(
              "No active device records found for this number. Registering new token...");
  ...
  Future<Map<String, dynamic>?> fetchServerTokenData(
      String mobileNumber) async {
    final url = Uri.parse(
        '$fcmUserTokensUrl?mobile_number=$mobileNumber');
  ...
  Future<void> sendTokenToBackend(
    ...
    final url = Uri.parse(fcmSaveTokenUrl);
  ...
  Future<void> updateTokenOnBackend(
      String mobileNumber, String newToken) async {
    final url = Uri.parse(fcmUpdateTokenUrl);
